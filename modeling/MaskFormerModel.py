...
class MaskFormerModel(nn.Module):
    ...
    # def forward(self, inputs,layer1,layer2,layer3,layer4,mask1):
    def forward(self, inputs, layer1, layer2, layer3, layer4):
        # features = self.backbone(inputs)
        myfeature =  {}
        myfeature['res2'] = layer1
        myfeature['res3'] = layer2
        myfeature['res4'] = layer3
        myfeature['res5'] = layer4
        outputs,multi_scale_features = self.sem_seg_head(myfeature)
        # outputs = self.sem_seg_head(myfeature, mask1)
        # outputs = self.sem_seg_head(features)

        # outputs = self.sem_seg_head(features,mask1)
        return outputs,multi_scale_features

    # def forward(self, inputs,infeatures):
    #     # features = self.backbone(inputs)
    #     # itoutputs = self.sem_seg_head(features)
    #     myoutputs = self.sem_seg_head(infeatures)
    #     # return itoutputs,myoutputs
    #     return myoutputs