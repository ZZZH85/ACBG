...
# 能否修改以下函数，使其能够根据提供的mask的位置来进行random copymove：
class RandomCopyMove(DualTransform):
    ...
    def _get_random_window(
        ...
        l_min_h = int(img_height * self.min_h)   #256*0.05=12
        l_min_w = int(img_width * self.min_w)   #384*0.05=19
        l_max_h = int(img_height * self.max_h)   #256*0.5=128
        l_max_w = int(img_width * self.max_w)  #384*0.5=192
        ...
        # position of left up corner of the window  窗口的位置
    ...
    def apply(self, img: np.ndarray, **params) -> np.ndarray:
        ...
        # Ensure that copy_region and paste_region do not overlap
        # while (abs(self.p_pos_h - c_pos_h) < c_window_h and abs(self.p_pos_w - c_pos_w) < c_window_w):
        #     c_pos_h, c_pos_w, c_window_h, c_window_w = self._get_random_window(H, W)
        #     self.p_pos_h, self.p_pos_w, self.p_window_h, self.p_window_w = self._get_random_window(H, W, c_window_h,
        #                                                                                            c_window_w)
        ...
        while overlap:
            overlap = False
            if (
                    abs(self.p_pos_h - c_pos_h) < c_window_h and abs(self.p_pos_w - c_pos_w) < c_window_w
            ):
                overlap = True
                c_pos_h, c_pos_w, c_window_h, c_window_w = self._get_random_window(H, W)
                self.p_pos_h, self.p_pos_w, self.p_window_h, self.p_window_w = self._get_random_window(H, W, c_window_h,
...
def get_albu_transforms(type_='train', outputsize=1024):
    ...
    if type_ == 'train':
        trans = albu.Compose([
            ...
            # RandomCopyMove(max_h=0.8, max_w=0.8,p=0.5),
            # RandomInpainting(max_h=0.8, max_w=0.8,p=0.5),

            # RandomCopyMove(max_h=0.5, max_w=0.5, p=0.8),
            # RandomInpainting(max_h=0.5, max_w=0.5, p=0.8),

            # RandomCopyMove(max_h=0.5, max_w=0.5, min_h=0.05,min_w=0.05,p=0.5),
            # RandomInpainting(max_h=0.5, max_w=0.5, min_h=0.05,min_w=0.05, p=0.5),

            RandomCopyMove(max_h=0.5, max_w=0.5,  p=0.5),
            RandomInpainting(max_h=0.5, max_w=0.5,  p=0.5),
    ...
    if type_ == 'test':
        ...
        trans = albu.Compose([
            ...
            # RandomCopyMove(p=0.1),
            # # RandomInpainting(p=0.1),
            # RandomCopyMove(p=0.5),
            # RandomInpainting(p=0.5),

            # RandomCopyMove(max_h=0.5, max_w=0.5, min_h=0.05, min_w=0.05, p=0.2),
            # RandomInpainting(max_h=0.5, max_w=0.5, min_h=0.05, min_w=0.05, p=0.2),
