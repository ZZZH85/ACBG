...
class PointFlowModuleWithMaxAvgpool(nn.Module):
    ...
    def forward(self, x, noise_x):
        ...
        ###noise   把noise变为和x_high一样的大小，代替edge_pred
        # N_noise, C_noise, H_noise, W_noise =  noise_x.shape
        noise_x_map = F.interpolate(noise_x, size=x_high.size()[2:], mode='bilinear', align_corners=True)

        point_indices, point_coords = get_uncertain_point_coords_on_grid(noise_x_map, num_points=self.edge_points)
        ...
        avgpool_grid = F.interpolate(avgpool_grid, size=(map_h, map_w), mode="bilinear", align_corners=True)#池化后8×8大小，差值成x_high大小
        # edge part
        x_high_edge = x_high - x_high * avgpool_grid #边界 Flb
        edge_pred = self.edge_final(x_high_edge)#卷积BN、RELU、卷积，执行完1通道，与x_high尺寸一样  [4,1,16,16]
