...
def get_uncertain_point_coords_on_grid(uncertainty_map, num_points):
    ...
    # num_points = max(H * W, num_points)
    # all_batch_point_indices = []
    # for batch_idx in range(R):
    #     # 获取当前批次的不确定性地图
    #     current_batch_map = uncertainty_map[batch_idx].view(H*W)
    #     # 找到不为0的点的索引
    #     point_indices = torch.nonzero(current_batch_map)
    #     # 添加到索引列表
    #     all_batch_point_indices.append(point_indices)
    # # nonzero_indices = torch.nonzero(uncertainty_map)
...
class PositionEmbeddingLearned_high(nn.Module):
    """
    ...
    def forward(self,x):
        ...
        furture_fosion = x+pos

        return furture_fosion

class PositionEmbeddingLearned_low(nn.Module):
    """
    Absolute pos embedding, learned.
    """
    def __init__(self, num_pos_feats=256):
        ...
        self.reset_parameters()
        self.down = nn.Conv2d(2 * num_pos_feats, num_pos_feats, 1)
...
class PointFlowModuleWithMaxAvgpool(nn.Module):
    ...
    def forward(self, x):
        ...
        #选点进行cross attention
        # x_high
        # x_high_pe = self.learnedpe_high(x_high)
        high_edge_feat = point_sample(x_high, point_coords)
        low_edge_feat = point_sample(x_low, point_coords)

        #x_low
        # x_low_pe = self.learnedpe_low(x_low)
        # # high_edge_feat = point_sample(x_high, point_coords)
        # low_edge_feat = point_sample(x_low_pe, point_coords)
