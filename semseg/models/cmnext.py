...
#     model.init_pretrained('/home/zhanghao/code/model/segformer/mit_b2.pth')
#     x = [torch.zeros(1, 3, 1024, 1024), torch.ones(1, 3, 1024, 1024), torch.ones(1, 3, 1024, 1024)*2, torch.ones(1, 3, 1024, 1024) *3]
#     # x = [torch.zeros(1, 3, 512, 512), torch.ones(1, 3, 512, 512), torch.ones(1, 3, 512, 512) * 2,
#     #      torch.ones(1, 3, 512, 512) * 3]
#     y = model(x)
#     print(y.shape)
...
class CMNeXt(BaseModel):
    def __init__(self, backbone: str = 'CMNeXt-B0',
                 modals: list = ['img', 'depth', 'event', 'lidar']) -> None:
        super().__init__(backbone, modals)
...
if __name__ == '__main__':
    modals = ['img', 'depth', 'event', 'lidar']
    model = CMNeXt('CMNeXt-B2', modals)
    model.init_pretrained('/mnt/csip-112/model/mit_b2.pth')
