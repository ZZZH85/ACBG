...
class myEncoderDecoder(nn.Module):
    ...
    def encode_decode(self, rgb, modal_x):
        ...
        # cmx
        x = self.backbone(rgb, modal_x)      #提取的四层特征
        out, feats = self.decode_head(x, return_feats=True)   #用于对输入的多个层级的特征进行解码和融合，返回预测结果和最终特征。
  ...
  #x为encoder输出的四层特征。
    ...
    def forward(self, rgb):
        ...
        if 'NP++' in self.mods:
            ...
            modal_x = torch.tile(modal_x, (3, 1, 1))  # 对tensor进行复制。(3, 1, 1)为制定维度。
        else:
            modal_x = None

        if self.prepro is not None:
            rgb = self.prepro(rgb)   #归一化处理。
